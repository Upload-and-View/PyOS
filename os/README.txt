PyOS:
Description:
An "operating system" on Python. To use it, you must use build.sh,
which automatically starts whenever installer is running.

    For now, only compatible for Debian-based Linux distributions, we
    will add soon compatibility for other distributions. PS: Don't
    use Windows build (build.bat), as it is out of date and does not 
    builds system fully. 
    
    How to build system manually?
        If you are on Windows, or on other distribution of Linux
        which is not based off Ubuntu, you might use this method:
    ...
    1. Build main stuff.
        ...
        Go to _home and make your a folder with name of your user
        you want to have, for example, I would use an user "gg".
    ...
    2. Build stuff that really needs to be in your system.
        --------------------------------------------------------------------
        Before you continue, make sure openssl tool is installed
        or try to use online SHA-512 encoder.
        ...
        For Windows:
            ...
            2. Use this command AS ADMINISTRATOR: choco install openssl
        
        For macOS:
            1. Install Homebrew (if you don't have it):
                       /bin/bash -c "<span class="math-inline">\(curl \-fsSL \[https\://raw\.githubusercontent\.com/Homebrew/install/HEAD/install\.sh\]\(https\://raw\.githubusercontent\.com/Homebrew/install/HEAD/install\.sh\)\)"</18\>
	...
	and yourpassword hash is hash of password you got using this command
	print((openssl passwd -6 "$yourpasswordhere")
        ...
        You can change defaultuser to administrator or vice versa, just leave on 
        root account administrator.
        
        Done!
        
    3. Build stuff that you probally will not need.
